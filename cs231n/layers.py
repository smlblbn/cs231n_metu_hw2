    ...
    for j in xrange(F): # jth filter
      ...
      for k in xrange(Hp):
        ...
        for l in xrange(Wp):
          ...
          # FILL IN THIS PART 
          #
          dx[i, :, hs:hs + HH, ws:ws + WW] = dx[i, :, hs:hs + HH, ws:ws + WW] + np.dot(w[j], dout[i, j, k, l])
          dw[j] = dw[j] + np.dot(padded[i, :, hs:hs + HH, ws:ws + WW], dout[i, j, k, l])
    ...
    for j in xrange(C):
      for k in xrange(Hp):
        ...
        for l in xrange(Wp):
          ...
          # FILL IN THIS PART 
          # 
          argmax_idx = np.argmax(x[i, j, hs:hs + HH, ws:ws + WW])
          dx[i, j, hs + argmax_idx / WW, ws + argmax_idx % HH] = dout[i, j, k, l]
  return dx
